...
__all__ = [
    ...
    "use_style", "model_colors", "plot_series", "plot_power", "plot_coherence",
    "plot_global_spectra", "plot_skill_heatmap",
]
...
def _period_axis(ax, lo: float, hi: float, ticks, label: str) -> None:
    ...
    ax.set_yticks(ticks)
    ax.set_yticklabels([_fmt_period(t) for t in ticks])
...
def plot_power(res: CWTResult, *, time=None, ax=None, alpha: float | None = None,
    ...
    """Wavelet power relative to white noise, log2 scale, with AR1 significance."""
    ax = ax or plt.gca()
    x, is_date = _xvalues(res.n, res.dt, time)
    p = res.normalized_power()
    lp = np.log2(np.maximum(p, 1e-12))
    ...
    _period_axis(ax, per.min(), per.max(), period_ticks, period_label)
    _finish_x(ax, is_date)
    if colorbar:
        _colorbar(ax, cf, "log2(power / variance)")
    if title:
        ax.set_title(title)
    return cf
...
def plot_coherence(coh: CoherenceResult, *, time=None, ax=None, sig: np.ndarray | None = None,
    ...
    if arrows:
        ...
        pj = np.unique(np.linspace(0, per.size - 1, arrow_grid[1] + 2)[1:-1].round().astype(int))
...
def plot_global_spectra(results: Mapping[str, CWTResult], *, obs_key: str = "obs", ax=None,
    ...
    ticks = (2.0 ** np.arange(np.ceil(np.log2(per.min())), np.floor(np.log2(per.max())) + 1)
             if period_ticks is None else np.asarray(period_ticks, float))
    ax.set_xticks(ticks)
    ax.set_xticklabels([_fmt_period(t) for t in ticks])
...
_METRICS = {
    ...
    "std_ratio": dict(cmap=DIVERGING, norm=TwoSlopeNorm(vmin=-2.0, vcenter=0.0, vmax=2.0),
                      label="log2 amplitude ratio", transform=np.log2),
...
def plot_skill_heatmap(df: pd.DataFrame, metric: str = "nse", *, ax=None,
                       ...
                       annotate: bool = True, fmt: str = "{:.2f}", text_metric: str | None = None,
                       title: str | None = None, colorbar: bool = True):
    ...
    cells (e.g. colour by ``phase_deg``, print ``lag`` in days).
    """
    ax = ax or plt.gca()
    ...
    ax.set_xticklabels(bands, rotation=0, fontsize=9)
